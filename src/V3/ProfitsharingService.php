...
class ProfitsharingService extends BaseService
{
    ...
    public function addReceiver(string $type, string $account, ?string $name = null)
    {
        if (empty($type) || empty($account)) {
            throw new Exception('分账接收方类型和账号不能为空');
        }
    ...
    public function submit(array $params)
    {
        ...
        if (!empty($this->subMchId)) {
            $publicParams['sub_mchid'] = $this->subMchId;
        }
    ...
    {
        if (empty($out_order_no) || empty($transaction_id)) {
            throw new Exception('商户分账单号和微信订单号不能为空');
        }
    ...
    {
        if (empty($out_return_no) || empty($out_order_no)) {
            throw new Exception('商户回退单号和商户分账单号不能为空');
        }
